# Set the working directory to the folder with this file
base::setwd("."); getwd()

# Sourcing functions
source("./Z_new_maxent_funcs.R")
...
# It is advised to update the R package "wallace" to its latest version from GitHub
# devtools::install_github("wallaceEcoMod/wallace")
library(wallace)

# For loop that for each species in a species.list thins the occurrences, extracts the 
# environmental values from the chosen layers, generates and saves a maxent model and a .csv file with the niche and model statistics
...
#SoilGrids <- raster::stack(list.files(path="/Users/marco/Desktop/rerun_trees_zip/var_SoilGrids/", pattern='*.tif$*', full.names=TRUE)) %>%
#  raster::mask(., CHELSA[[1]])
#
#CHELSA2 <- raster::stack(list.files(path="/Users/marco/Desktop/rerun_trees_zip/var_CHELSA_adj_res/", pattern='*.tif$*', full.names=TRUE)) %>%
#  raster::mask(., SoilGrids[[1]]) %>%
#  raster::stack(., SoilGrids) %>% 
#  raster::brick(.)
#
### Removing the . from the CHELSA layer names
#names(CHELSA2)[1:19] <- gsub("\\.", "_", names(CHELSA2)[1:19])
##
#raster::writeRaster(CHELSA2, filename = names(CHELSA2), bylayer = TRUE, format = "GTiff")
...
#Load occurence data (long,lat for maxent)
cleaned_species_list <- read_csv("./var_config/list_herbs_cleaned.csv")

modelled <- list.files(path ="./../3_generated_data/res_maxent_result_tables/") %>%
  sub("_maxent_results.csv", "", .)
dropped <- list.files(path ="./../3_generated_data/res_dropped_species/") %>%
  sub("_dropped.csv", "", .)

species_to_model <- cleaned_species_list$Species %>%
  setdiff(., modelled) %>%
  setdiff(., dropped)

ds <- read_csv("./../1_original_data/herb_occs_elevation-105450sp.csv")

run.maxnet(occurrences_df = ds,
           species_list = "Campanula_thyrsoides", 
           env_values = envs)

parallel::mclapply(rev(species_to_model), function(species) {
  run.maxnet(species_list = species, 
             env_values = envs)
  }, mc.cores = 4)
